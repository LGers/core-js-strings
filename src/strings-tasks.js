...
function removeFirstOccurrences(str, value) {
  ...
  if (index >= 0) {
    start = str.slice(0, index);
    end = str.slice(index + value.length, str.length + 1);
  }
...
function isPalindrome(str) {
  const strClear = str.replace(/[^A-Z0-9]/gi, '').toLocaleLowerCase();
  const rev = [...strClear].reverse().join('');
  return strClear === rev;
}
...
function findLongestWord(sentence) {
  const word = sentence.split(' ').reduce((acc, cur) => {
...
function invertCase(str) {
  const invertedString = [...str]
    .map((char) => {
      ...
      return upperChar === char ? char.toLowerCase() : upperChar;
    })
    .join('');
